...
```{r, echo=TRUE, show=FALSE, warnings=FALSE}
rm(list=ls(all=TRUE)) 
```
...
group<-rearing[which(rearing$Community == "Group"),]
group_final<-group[which(group$Timepoint == "Final"),]
```
...
```{r, results=TRUE, warnings=FALSE}
survmod2a<-glmer(Survivorship ~ Richness + Fusion.Partners + Richness:Fusion.Partners + (1|Parent.Site/Colony) + (1|Tank) + (1|Slide), data=final, family=binomial, subset=c(Community=="Group"))
...
Generate mean values of survivorship for 1) number of juveniles and 2) within "group" juveniles, mean values for number of fusion partners.   
```{r, results=TRUE, warnings=FALSE}
...
Analyze fusion as a product of genotypic richness within "group" juveniles.   
```{r, results=TRUE, warnings=FALSE}
#with final dataset
...
```{r, results=TRUE, warnings=FALSE}
grow_out_figs<-plot_grid(surv.effects2b, labels = c(""), ncol=1, nrow=1, rel_heights= c(1), rel_widths = c(1), label_size = 20, label_y=1, align="h");grow_out_figs

ggsave(filename="Figures/grow_out_figs.png", plot=grow_out_figs, dpi=500, width=8, height=8, units="in")
```
...
Subset the final timepoint, "group" juvenile datasets for additional analyses.  
```{r, results=TRUE, warnings=FALSE}
...
First, examine sample size of groups. As there were very few "not fused" individuals in group juvenile slides, these will be removed from the analysis. We will only analyze survivorship of "individuals" and those which are "fused". We will then examine the differences between "single" and "multiple" genotype groups.  
...
ggsave(filename="Figures/stress_figs.pdf", plot=stress_figs, dpi=500, width=20, height=6, units="in")
```
...
```{r, restults=TRUE, warnings=FALSE}
