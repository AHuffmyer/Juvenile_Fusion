...
Analyze over the effect of time.  
```{r, results=FALSE}
#with full dataset
survmod1<-glmer(Survivorship ~ Day + Richness + Community + Fusion.Partners + Richness:Fusion.Partners + Day:Community + Day:Richness + Day:Fusion.Partners + Day:Richness:Fusion.Partners + (1|Parent.Site/Genotype) + (1|Tank) + (1|Slide/Sample), data=rearing, family=binomial)

summary(survmod1)
Anova(survmod1, type="II")

plot(allEffects(survmod1))

dispersion_glmer(survmod1) #no overdispersion
plot(residuals(survmod1)) #Dispersed randomly
```


Analyze with final dataset.  
```{r, results=TRUE}
#with final dataset
...
Analyze fusion as a product of genotypic richness and total number of individuals on the slide. 
```{r, results=FALSE}
#with final dataset
...
Analyze polyp expansion with the final dataset.    
...
Check for effect of community type.  
...
Plot effect of community over time.  
...
Create plot for effect of temperature in multiple communities.      
```{r, results=TRUE}
...
Analyze fusion over the course of the stress test.  
...
No effects at the end of the experiment.  

## 6. STRESS GROWTH  

Analyze with a poisson regression model.   

Analyze growth during the stress test. 
...
```{r, results=FALSE}
#with full dataset 
polypmod2a<-glmer(Polyps^2 ~ Day * Fusion.Partners * Richness * Treatment + (1|Parent.Site/Genotype) + (1|Treatment:Tank) + (1|Slide/Sample), data=stress, family = poisson, subset=c(Community=="Multiple"))
...
meanstressgrowth2 <- ddply(multiple_final_stress, c("Treatment", "Fusion.Partners", "Richness"), summarise,
                   N    = length(Polyps[!is.na(Polyps)]),
                   mean = mean(Polyps, na.rm=TRUE),
                   sd   = sd(Polyps, na.rm=TRUE),
                   se   = sd / sqrt(N), 
...
ggsave(filename="Figures/New/stress_figs.png", plot=stress_figs, dpi=500, width=20, height=6, units="in")
```


## 7. TEMPERATURE DATA

Load in temperature data from Apex files.    
...
Plot by time of day by summarizing the mean temp at each time of day for each tank. Display the mean temperatures of each treatment during the Ambient and Stress phases.   

```{r, results=FALSE}
QuarterlyJ <- ddply(juvtemps, c("TimeDay", "Treatment"), summarise,
                   N    = length(Temperature[!is.na(Temperature)]),
                   mean = mean(Temperature, na.rm=TRUE),
                   sd   = sd(Temperature, na.rm=TRUE),
                   se   = sd / sqrt(N), 
