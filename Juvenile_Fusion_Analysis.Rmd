---
title: "Tissue fusion provides a survival advantage during thermal stress in juvenile Pocillopora acuta corals"
author: "AS Huffmyer (R Markdown author), C Drury, E Majerova, JD Lemus, RD Gates"
date: "2020"
output:
  html_document:
    code_folding: hide
...
Generate mean values of survivorship for 1) community size (individuals or groups); 2) within "group" juveniles, mean values for number of fusion partners; and 3) within "group" juveniles, mean values for genotypic richnes.   
...
First, examine sample size of groups. As there were very few "not fused" individuals in group juvenile slides, these were removed from the analysis. We will only analyze survivorship of "individuals" and those which are "fused". We will then examine the differences between "single" and "multiple" genotype groups.   
...
Next we analyze  the effect of genotypic richness, fusion, and temperaturetreatment on survivorship over the course of the stress test. Note that there is no interaction between richness type and fusion type as individual juveniles are not exposed to other juveniles and therefore no option for this interaction with genotypic richness.   
...
## **6. Stress Period: Fusion Persistence**    
...
## **7. Stress Period: Polyp Expansion (Growth)**    
...
## **8. Stress Period Figures**    

Combine figures for Stress Period:   

```{r, results=TRUE, warning=FALSE}
stress_figs<-plot_grid(stress.effectsFUSE, stress.effectsRICHNESS, labels = c("A", "B"), ncol=2, nrow=1, rel_heights= c(1,1), rel_widths = c(1,1), label_size = 20, label_y=1, align="h")
...
## **9. Physical Data: Temperature Conditions**    
